...
@Serializable
data class GameScreenRoute(val topic: String) : NavKey // Змінили на data class
    ...
    NavDisplay(
        ...
        entryProvider = entryProvider {
            entry<EntryScreenRoute> {
                EntryScreen(onGameScreen = { topic ->
                    // Передаємо введену тему в маршрут
                    backStack.add(GameScreenRoute(topic))
                })
            }
            entry<GameScreenRoute> { route ->
                // Отримуємо тему напряму з об'єкта route
